...
#Reading training data for Images
...
for file in os.listdir(path):
    if file.endswith("_entropy"):
        
        #Opening the file & reading
        f = np.loadtxt(f"{path}/{file}")
        f = f.flatten()

        #Appending classes of training data set to y_train
        if file[0] == 'c':
            y_train.append(file[0:5])
        elif file[0] == 'f':
            y_train.append(file[0:6])
        elif file[0] == 'h':
            y_train.append(file[0:7])
        elif file[0] == 'm':
            y_train.append(file[0:8])
        elif file[0] == 'o':
            y_train.append(file[0:11])
        
        #Combining all tranining samples in global variable x_train
        if flag == 0:
            globals()['x_train'] = f
        else:
            globals()['x_train'] = np.row_stack((x_train,f))
        flag = 1
            
x_train = x_train/x_train.max()  #Normalizing training data set

#Reading test data for Images
path = 'Image/dev'
...
for file in os.listdir(path):
    if file.endswith("_entropy"):
        ...
        #Appending classes of training data set to y_test & creating list of class labels for roc & det curves
        if file[0] == 'c':
            truth.append(0)
            y_test.append(file[0:5])
        elif file[0] == 'f':
            truth.append(1)
            y_test.append(file[0:6])
        elif file[0] == 'h':
            truth.append(2)
            y_test.append(file[0:7])
        elif file[0] == 'm':
            truth.append(3)
            y_test.append(file[0:8])
        elif file[0] == 'o':
            truth.append(4)
            y_test.append(file[0:11])

        #Combining all test samples in global variable x_test
        if flag == 0:
            globals()['x_test'] = f
        else:
            globals()['x_test'] = np.row_stack((x_test,f))
        flag = 1
...
x_train_pca = np.dot(x_train - np.mean(x_train , axis = 0), PCA_components(x_train, 80).real)
x_test_pca = np.dot(x_test - np.mean(x_test , axis = 0), PCA_components(x_train, 80).real)
...
x_train_lda = np.dot(x_train, LDA_components(x_train, y_train, 80).real)
x_test_lda = np.dot(x_test, LDA_components(x_train, y_train, 80).real)

#Function for confusion Matrix
def confusion(y_true, y_pred, model):
    val= ['coast', 'forest', 'highway', 'mountain','opencountry']  #Class labels
    ...
    ax.set_title('Confusion matrix by %s'% model+" for Image data")    
...
class KNN_function:
    ...
    def prob(self, neighbour):
        t = Counter(neighbour)
        self.s[self.i, 0] = t['coast']
        self.s[self.i, 1] = t['forest']
        self.s[self.i, 2] = t['highway']
        self.s[self.i, 3] = t['mountain']
        self.s[self.i, 4] = t['opencountry']
...
print("Applying KNN for Image data")
...
def apply_logreg(x_train, y_train, x_test):
    ...
    score[:,0:1] *= (y_test.count("coast")/len(y_test))
    score[:,1:2] *= (y_test.count("forest")/len(y_test))
    score[:,2:3] *= (y_test.count("highway")/len(y_test))
    score[:,3:4] *= (y_test.count("mountain")/len(y_test))
    score[:,4:5] *= (y_test.count("opencountry")/len(y_test))

    return y_pred, score

#Computing scores & plotting confusion matrix for Logistic Regression classifier
print("Applying Logistic Regression for Image data")
...
print("Applying Logistic Regression using PCA for Image data")
score_logreg_pca = find_score_and_plot_confusion(x_train_pca, y_train, x_test_pca, "Logistic Regression", " using PCA")
print("Applying Logistic Regression using LDA for Image data")
...
print("Applying SVM for Image data")
...
print("Applying ANN for Image data")
...
#ROC & DET curves
print("Plotting ROC & DET curves for Image data")
plot_all_classifiers_together(score_knn, score_logreg, score_svm, score_ann)
#ROC & DET curves using PCA
print("Plotting ROC & DET curves using PCA for Image data")
...
print("Plotting ROC & DET curves using LDA for Image data")
