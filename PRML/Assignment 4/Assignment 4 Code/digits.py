...
#Looping over the folder to access all files in it
for file in os.listdir(path):
    if file.endswith(".mfcc"):
        ...
        y_train.append(file[3])

        #Converting each training data into matrix of floating point numbers
        t = []
        for i in range(nf):
            y = f.readline().split()
            for i in range(nc):
        ...
        #Equating all data sets length to 3800 by resampling
        t = resample(t,3800)
...
#Reading test data for Isolated spoken digits
path = 'Digits/dev'
...
x_train_pca = np.dot(x_train - np.mean(x_train , axis = 0), PCA_components(x_train, 300).real)
x_test_pca = np.dot(x_test - np.mean(x_test , axis = 0), PCA_components(x_train, 300).real)
...
x_train_lda = np.dot(x_train, LDA_components(x_train, y_train, 300).real)
x_test_lda = np.dot(x_test, LDA_components(x_train, y_train, 300).real)

#Function for confusion Matrix
def confusion(y_true, y_pred, model):
    val= ['2', '6', '7', '9', 'o']  #Class labels
    ...
    ax.set_title('Confusion matrix by %s'% model+" for Speech data")    
...
print("Applying KNN for Speech data")
...
print("Applying KNN using PCA for Speech data")
score_knn_pca = find_score_and_plot_confusion(x_train_pca, y_train, x_test_pca, "KNN", " using PCA")
print("Applying KNN using LDA for Speech data")
...
def apply_logreg(x_train, y_train, x_test):
    ...
    score[:,0:1] *= (y_test.count("2")/len(y_test))
    score[:,1:2] *= (y_test.count("6")/len(y_test))
    score[:,2:3] *= (y_test.count("7")/len(y_test))
    score[:,3:4] *= (y_test.count("9")/len(y_test))
    score[:,4:5] *= (y_test.count("o")/len(y_test))
    
    return y_pred, score

#Computing scores & plotting confusion matrix for Logistic Regression classifier
print("Applying Logistic Regression for Speech data")
...
print("Applying Logistic Regression using PCA for Speech data")
score_logreg_pca = find_score_and_plot_confusion(x_train_pca, y_train, x_test_pca, "Logistic Regression", " using PCA")
print("Applying Logistic Regression using LDA for Speech data")
...
print("Applying SVM for Speech data")
...
print("Applying SVM using PCA for Speech data")
score_svm_pca = find_score_and_plot_confusion(x_train_pca, y_train, x_test_pca, "SVM", " using PCA")
print("Applying SVM using LDA for Speech data")
...
print("Applying ANN for Speech data")
...
print("Applying ANN using PCA for Speech data")
score_ann_pca = find_score_and_plot_confusion(x_train_pca, y_train, x_test_pca, "ANN", " using PCA")
print("Applying ANN using LDA for Speech data")
...
#ROC & DET curves
print("Plotting ROC & DET curves for Speech data")
plot_all_classifiers_together(score_knn, score_logreg, score_svm, score_ann)
#ROC & DET curves using PCA
print("Plotting ROC & DET curves using PCA for Speech data")
...
print("Plotting ROC & DET curves using LDA for Speech data")
